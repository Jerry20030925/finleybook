...
export default function Navigation() {
  ...
  const navigation = [
    ...
    // Subscription moved to profile, removing from main nav if desired, 
    // but user only said "move automatic subscription interface to profile", 
    // they didn't explicitly say remove the link. 
    // However, usually "move" implies removing the old one.
    // I will keep it for now as "Subscription" page might be different from "Manage Subscription".
    // Actually, SubscriptionPage.tsx is the sales page. Profile has the status.
    // I'll leave it as is for now to avoid breaking changes unless requested.
            ...
            <>
                ...
                <div className="flex h-16 justify-between">
                  ...
                  <div className="hidden md:ml-4 md:flex md:items-center md:space-x-4">
                    <button
                      ...
                      className="flex items-center gap-1 rounded-full bg-indigo-50 px-3 py-1.5 text-sm font-medium text-indigo-600 hover:bg-indigo-100 transition-colors"
                    >
                      <GiftIcon className="h-4 w-4" />
                      <span>{language === 'en' ? 'Invite' : '邀请'}</span>
                    </button>
              ...
              <Disclosure.Panel className="md:hidden">
                <div className="space-y-1 pb-3 pt-2">
                  ...
                  <button
                    onClick={() => setShowInviteModal(true)}
                    className="w-full text-left border-transparent text-indigo-600 hover:bg-indigo-50 hover:text-indigo-800 block border-l-4 py-2 pl-3 pr-4 text-base font-medium"
                  >
                    {language === 'en' ? 'Invite Friends' : '邀请好友'}
                  </button>
                </div>
