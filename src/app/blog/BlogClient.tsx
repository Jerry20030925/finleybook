    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gray-50">
                ...
                <section className="bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-20 pb-12">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center">
                            ...
                            >
                                Actionable playbooks for bookkeeping, budgeting, and monthly financial decision quality.
                                Built for users who want clear execution, not vague advice.
                            </motion.p>
                ...
                {/* Trending Wealth Opportunities (Internal Linking SEO) */}
                <section className="py-8 bg-indigo-900 text-white overflow-hidden relative">
                    <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10" />
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
                                    <TagIcon className="h-8 w-8 text-indigo-300" />
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold">Trending Wealth Opportunities</h2>
                                    <p className="text-indigo-200 text-sm">High-signal products and opportunities curated for disciplined financial planning.</p>
                                </div>
                            </div>
                            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 w-full md:w-auto">
                                {PRODUCTS.slice(0, 3).map((product) => (
                                    <Link
                                        key={product.id}
                                        href={`/wealth/product/${product.id}`}
                                        className="shrink-0 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10 transition-all flex items-center gap-3 group"
                                    >
                                        <img src={product.image} alt={product.name} className="h-10 w-10 rounded-lg object-cover" />
                                        <div>
                                            <div className="text-xs font-bold truncate max-w-[120px]">{product.name}</div>
                                            <div className="text-[10px] text-indigo-300">Opportunity score by Finley AI</div>
                                        </div>
                                        <ArrowRightIcon className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                                    </Link>
                                ))}
                                <Link href="/wealth" className="shrink-0 flex items-center gap-2 text-sm font-bold hover:text-indigo-300 transition-colors pl-4">
                                    View Vault <ArrowRightIcon className="h-4 w-4" />
                                </Link>
                            </div>
                        </div>
                    </div>
