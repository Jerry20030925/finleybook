...
export default function ShopPageClient() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20">
                    ...
                    {MERCHANTS.map((merchant, index) => (
                        ...
                        >
                            <div className="flex items-center justify-between mb-4">
                                <div className={`h-12 w-12 rounded-full flex items-center justify-center ${merchant.color} p-2 relative`}>
                                    <Image
                                        src={merchant.logo}
                                        alt={merchant.name}
                                        width={48}
            ...
            {/* Trending Products to ensure crawlability */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
                <h2 className="text-2xl font-bold text-gray-900 mb-8">Trending Deals</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
                    {PRODUCTS.slice(0, 8).map((product) => (
                        <Link
                            key={product.id}
                            href={`/wealth/product/${product.id}`}
                            className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition-all group"
                        >
                            <div className="aspect-square relative mb-4 bg-gray-50 rounded-lg overflow-hidden p-4">
                                <Image
                                    src={product.image}
                                    alt={product.name}
                                    fill
